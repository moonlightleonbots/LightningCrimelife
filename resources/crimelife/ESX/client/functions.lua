...
function ESX.OpenContext(...)
    return contextAvailable and exports["esx_context"]:Open(...) or not contextAvailable
end

function ESX.PreviewContext(...)
    return contextAvailable and exports["esx_context"]:Preview(...) or not contextAvailable
end

function ESX.CloseContext(...)
    return contextAvailable and exports["esx_context"]:Close(...) or not contextAvailable
end

function ESX.RefreshContext(...)
    return contextAvailable and exports["esx_context"]:Refresh(...) or not contextAvailable
end
...
function ESX.Game.Teleport(entity, coords, cb)
    local vector = type(coords) == "vec4" and coords or type(coords) == "vec3" and vec4(coords, 0.0) or
...
function ESX.Game.SpawnObject(object, coords, cb, networked)
    ...
    local vector = type(coords) == "vec3" and coords or vec(coords.x, coords.y, coords.z)
...
local function EnumerateEntitiesWithinDistance(entities, isPlayerEntities, coords, maxDistance)
    ...
    for k, entity in next, (entities) do
...
ESX.Game.GetClosestEntity = function(entities, isPlayerEntities, coords, modelFilter)
    ...
    for k, entity in next, (filteredEntities or entities) do
        local distance = #(coords - GetEntityCoords(entity))
...
function ESX.Game.SetVehicleProperties(vehicle, props)
    ...
    if props.extras ~= nil then
        for extraId, enabled in next, (props.extras) do
    ...
    if props.windowsBroken ~= nil then
        for k, v in next, (props.windowsBroken) do
            if v then
    ...
    if props.doorsBroken ~= nil then
        for k, v in next, (props.doorsBroken) do
            if v then
    ...
    if props.tyreBurst ~= nil then
        for k, v in next, (props.tyreBurst) do
            if v then
