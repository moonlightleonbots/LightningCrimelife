...
Core.PlayerFunctionOverrides.OxInventory = {
	...
	getInventoryItem = function(self)
		return function(name)
			return Inventory.GetItem(self.source, name)
		end
	end,

	addInventoryItem = function(self)
		return function(name, count, slot)
			return Inventory.AddItem(self.source, name, count or 1, slot)
		end
	end,

	removeInventoryItem = function(self)
		return function(name, count, slot)
			return Inventory.RemoveItem(self.source, name, count or 1, slot)
		end
	end,

	setInventoryItem = function(self)
		return function(name, count)
			return Inventory.SetItem(self.source, name, count)
		end
	end,

	canCarryItem = function(self)
		return function(name, count)
			return Inventory.CanCarryItem(self.source, name, count)
		end
	end,
	...
	syncInventory = function(self)
		return function(weight, maxWeight, items, money)
			...
			if money then
				for accountName, amount in next,(money) do
					local account = self.getAccount(accountName)
