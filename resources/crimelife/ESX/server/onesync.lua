...
---@param source vec3|number playerId or vec3 coordinates
...
function ESX.OneSync.SpawnObject(model, coords, heading, cb)
	...
	local objectCoords = type(coords) == "vec3" and coords or vec3(coords.x, coords.y, coords.z)
	CreateThread(function()
...
local function getNearbyEntities(entities, coords, modelFilter, maxDistance, isPed)
	...
	coords = type(coords) == 'number' and GetEntityCoords(GetPlayerPed(coords)) or vec3(coords.x, coords.y, coords.z)
...
---@param coords vec3
---@param modelFilter table models to ignore, where the key is the model hash and the value is true
---@return number entityId, number distance, vec3 coords
